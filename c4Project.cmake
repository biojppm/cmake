...
macro(_c4_handle_semantic_version version)
    ...
    c4_setg(PROJECT_SONAME ${_soname})
    ...
    c4_setg(${prefix}_SONAME ${_soname})
    ...
    c4_setg(${_c4_uprefix}VERSION_SONAME ${_soname})
endmacro()
...
function(c4_proj_get_version dir tag_o full_o major_o minor_o patch_o tweak_o)
    ...
    macro(_c4pgv_set_cmake_version)
        if(DEFINED ${_c4_prefix}_VERSION_FULL AND NOT "${${_c4_prefix}_VERSION_FULL}" STREQUAL "")
            set(full "${${_c4_prefix}_VERSION_FULL}")
        elseif(DEFINED PROJECT_VERSION AND NOT "${PROJECT_VERSION}" STREQUAL "")
            set(full "${PROJECT_VERSION}")
        else()
            set(full "0.0.0")
        endif()
        if(DEFINED ${_c4_prefix}_VERSION_MAJOR AND NOT "${${_c4_prefix}_VERSION_MAJOR}" STREQUAL "")
            set(major "${${_c4_prefix}_VERSION_MAJOR}")
        else()
            set(major "${PROJECT_VERSION_MAJOR}")
        endif()
        if(DEFINED ${_c4_prefix}_VERSION_MINOR AND NOT "${${_c4_prefix}_VERSION_MINOR}" STREQUAL "")
            set(minor "${${_c4_prefix}_VERSION_MINOR}")
        else()
            set(minor "${PROJECT_VERSION_MINOR}")
        endif()
        if(DEFINED ${_c4_prefix}_VERSION_PATCH AND NOT "${${_c4_prefix}_VERSION_PATCH}" STREQUAL "")
            set(patch "${${_c4_prefix}_VERSION_PATCH}")
        else()
            set(patch "${PROJECT_VERSION_PATCH}")
        endif()
        if(DEFINED ${_c4_prefix}_VERSION_TWEAK AND NOT "${${_c4_prefix}_VERSION_TWEAK}" STREQUAL "")
            set(tweak "${${_c4_prefix}_VERSION_TWEAK}")
        else()
            set(tweak "${PROJECT_VERSION_TWEAK}")
        endif()
        set(${tag_o}   "v${full}" PARENT_SCOPE)
    ...
    if(NOT GIT)
        c4_log("git was not found; using the CMake-declared project version for cpack")
        _c4pgv_set_cmake_version()
        return()
    endif()
    execute_process(COMMAND ${GIT} rev-parse --is-inside-work-tree
        ...
        OUTPUT_QUIET
        ERROR_QUIET)
    if(NOT git_work_tree_result EQUAL 0)
        c4_log("git metadata is unavailable; using the CMake-declared project version for cpack")
        _c4pgv_set_cmake_version()
        return()
    endif()
        ...
        elseif(${_LIBRARY})
            ...
            if("${_blt}" STREQUAL "INTERFACE")
                ...
                if("${_blt}" STREQUAL SHARED)
                    set_target_properties(${target} PROPERTIES SOVERSION ${${_c4_prefix}_SONAME})
                endif()
